...
const Popup = () => {
	...
	return (
		<div
			className={cn('fixed w-full h-full top-0 left-0 z-40 justify-center items-center bg-black/90 hidden', {
					...
					<div className='mb-6 flex items-start justify-between'>
						<div>
							...
							<p className='mt-1 text-sm text-zinc-400'>
								You can contact us through the following channels <br /> we are available 24/7.
							</p>
						</div>

						<button onClick={() => dispatch(setIsOpenContactPopup(false))} className='text-zinc-400 hover:text-white'>
							✕
						</button>
					</div>
						...
						{contact.map((item, index) => (
							...
							>
								<div className='flex items-center gap-3'>
									<div className='flex h-10 w-10 items-center justify-center rounded-lg bg-zinc-700 text-lg'>{item.icon}</div>
									<div>
